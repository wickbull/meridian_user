...
trait Translatable
{
     ...
     * @return \Illuminate\Database\Eloquent\Builder|static
     */
    public function scopeWhereTranslationAll($query, $key, $value)
    {
        return $query->whereHas('translations', function ($query) use ($key, $value) {
            $query->where($this->getTranslationsTable().'.'.$key, $value);
        });
    }
     ...
     * @return \Illuminate\Database\Eloquent\Builder|static
     */
    public function scopeWhereTranslationIn(Builder $query, $key, $values, $locale = null)
    {
        return $query->whereHas('translations', function (Builder $query) use ($key, $values, $locale) {
            $query->whereIn($this->getTranslationsTable().'.'.$key, $values);
